  ...
  <>
    ...
    <TouchableOpacity onPress={onPress} style={styles.touchableOpacity}>
      <CheckBox
        ...
        onPress={onPress}
      />
      <Text numberOfLines={1} ellipsizeMode="tail" style={styles.title}>{item.motorisation}</Text>
...
const SelectModel = ({ route, navigation, languageReducer }) => {
  ...
  // params Route
  const { motorisationModel } = route.params;

  // state
  const [datas, setDatas] = useState(motorisationModel);
  const [value, setValue] = useState('');

  // copy the state and filter the data
  let clone = [...datas];
  clone = value ? clone.filter((x) => x.motorisation.includes(value)) : clone;

  const carMotorisationSelection = datas.filter((y) => y.active === true);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.m20}>
        <Button
          onPress={() => navigation.navigate('Quotes', { carMotorisationSelection })}
          theme="dark"
          title={lg.rates.save}
        />
      </View>
